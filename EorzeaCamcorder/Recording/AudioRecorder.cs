...
public delegate void AudioCaptureCallback(IntPtr data, int size);
...
public unsafe class AudioRecorder : IDisposable
{
    ...
    public AudioRecorder() => CoInitializeEx(null, (uint)COINIT.COINIT_MULTITHREADED);
    ...
    public void StartToCallback(AudioCaptureCallback callback)
    {
        ...
        var handler = new ActivationHandler(opPtr =>
        {
            try
            {
                var asyncOp = (IActivateAudioInterfaceAsyncOperation*)opPtr;
                HRESULT hr;
                IUnknown* unknown = null;

                asyncOp->GetActivateResult(&hr, &unknown);
                if (hr < 0 || unknown == null) throw new COMException("Activation failed", hr);

                _audioClient = (IAudioClient*)unknown;
                ...
                if (_audioClient->GetMixFormat(&format) < 0 || format == null)
                {
                    ...
                    *format = new WAVEFORMATEX {
                        ...
                        nBlockAlign = 4, nAvgBytesPerSec = 192000, cbSize = 0
                    };
                }
                ...
                hr = _audioClient->Initialize(AUDCLNT_SHAREMODE.AUDCLNT_SHAREMODE_SHARED, 0x00020000, 10_000_000, 0, format, null);
                ...
                hr = _audioClient->GetService(&iid, (void**)&capture);
                if (hr < 0) throw new COMException("GetService(IAudioCaptureClient) failed", hr);

                _captureClient = capture;
            }
            catch (Exception ex) { activationEx = ex; }
            finally { activationEvent.Set(); }
        });
        ...
        try
        {
            var activationParams = new ActivationParams { ActivationType = 1, TargetProcessId = (uint)Process.GetCurrentProcess().Id, ProcessLoopbackMode = 0 };
            var prop = new PropVariantBlob { vt = 65, cbSize = (uint)sizeof(ActivationParams), pBlobData = (byte*)&activationParams };
            ...
            {
                int hr = ActivateAudioInterfaceAsync(deviceId, &iid, (PROPVARIANT*)&prop, (IActivateAudioInterfaceCompletionHandler*)handlerPtr, &asyncOp);
                if (hr < 0) throw new COMException("ActivateAudioInterfaceAsync failed", hr);
            }

            if (!activationEvent.Wait(5000)) throw new TimeoutException("Audio activation timed out.");
            if (activationEx != null) throw activationEx;
        }
        finally
        {
        ...
        _captureThread = new Thread(CaptureLoop) { IsBackground = true, Priority = ThreadPriority.Highest };
        _captureThread.Start();
    }

    private void CaptureLoop()
    {
        if (_audioClient->Start() < 0) return;
        ...
        {
            uint packetSize;
            if (_captureClient->GetNextPacketSize(&packetSize) < 0) break;
            ...
            while (packetSize > 0)
            {
                ...
                if (_captureClient->GetBuffer(&data, &frames, &flags, &devPos, &qpc) < 0) break;

                if (frames > 0 && _callback != null)
                    _callback((IntPtr)data, (int)frames * bytesPerFrame);
    ...
    {
        if (_sampleRate == 0) throw new InvalidOperationException("Format not initialized.");
    ...
    public void Dispose()
    {
        ...
        if (_captureClient != null) { _captureClient->Release(); _captureClient = null; }
        if (_audioClient != null) { _audioClient->Release(); _audioClient = null; }
